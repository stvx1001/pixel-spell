...
const LINKS: { id: string; label: string; icon: BitmapName }[] = [
...
export function FloatingNav() {
  ...
  return (
    <nav
      aria-label="Main"
      className="fixed bottom-4 left-1/2 z-50 flex max-w-[calc(100vw-16px)] -translate-x-1/2 items-center gap-0.5 rounded-full bg-white p-1.5 shadow-[0_12px_32px_rgba(58,42,16,0.12)] ring-1 ring-ink/8 md:bottom-7"
    >
      ...
      {LINKS.map(({ id, label, icon }) => {
        ...
        return (
          ...
          >
            ...
            <span className={on ? "hidden sm:inline" : "hidden lg:inline"}>{label}</span>
          </a>
        );
      })}
      ...
      <a
        href={BOOK_A_CALL}
        className="flex shrink-0 items-center gap-1.5 rounded-full bg-ink px-4 py-3 font-semibold whitespace-nowrap text-white transition-colors hover:bg-pink md:px-5"
      >
